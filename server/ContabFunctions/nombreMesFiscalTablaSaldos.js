...
const nombreMesFiscalTablaSaldos = (mesFiscal) => {

    check(mesFiscal, Match.Integer);

    let errorMessage;

    if (!Number.isInteger(mesFiscal)) {
        errorMessage = `Error: el valor pasado a la función 'nombreMesFiscalTablaSaldos' no es del tipo 'entero'.`;
    ...
    switch (mesFiscal) {
        ...
        case 12:
            ...
            break;
        default:
            errorMessage = `Error: el valor pasado a la función 'nombreMesFiscalTablaSaldos' no es un valor válido (1 a 12).`;
            return { error: true, errorMessage: errorMessage };
    }
...
export { nombreMesFiscalTablaSaldos }; 